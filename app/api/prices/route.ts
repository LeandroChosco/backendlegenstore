...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('prices')
    .select('game, server, region, price_usd, markup_pct, is_static, raw_data, updated_at')
    .order('game')
    .order('server')
  ...
  const applyMarkup = (price: number, pct: number) =>
    parseFloat((price * (1 + pct / 100)).toFixed(6))

  const result: Record<string, unknown> = {}

  for (const row of data) {
    const price_usd = applyMarkup(row.price_usd, row.markup_pct ?? 0)
